    ...
    <div className="space-y-4">
      ...
      {(() => {
        ...
        const init: TaoDonInitial = {
          ...
          ckSender: !!(eo.bankName || eo.bankAccountNo),
          bankName: eo.bankName ?? "",
          bankAccountNo: eo.bankAccountNo ?? "",
          bankAccountName: eo.bankAccountName ?? "",
          invoiceRequested: !!eo.invoiceRequested,
          invoiceTaxCode: eo.invoiceTaxCode ?? "",
...
function AssignToVehicleDialog({
  ...
  const presetBranch = useMemo(() => {
    const vals = [...new Set(selectedOrders.map((o) => (o.route || "").trim()).filter(Boolean))];
    return vals[0] ?? "";
  }, [selectedOrders]);
  const presetItinerary = useMemo(() => {
    const vals = [...new Set(selectedOrders.map((o) => (o.itinerary || "").trim()).filter(Boolean))];
    return vals[0] ?? "";
  }, [selectedOrders]);
      ...
      <DialogContent className="!flex max-h-[90vh] w-[calc(100vw-2rem)] max-w-5xl flex-col gap-4 overflow-hidden p-6">
        ...
        <div className="min-h-0 min-w-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden pr-1">
          <AssignVehiclePicker
            open={open}
            onPick={setPick}
            presetBranch={presetBranch}
            presetItinerary={presetItinerary}
          />
